...
See link:https://docs.gradle.com/develocity/compatibility/#build_scans[compatibility matrix] for details.
...
==== Remove deprecated members of the `org.gradle.util` package
...
===== Remove deprecated custom source set interfaces
The following source set interfaces have been removed:
...
Kotlin 2.1 removes support for targeting Kotlin language 1.4 and 1.5, it also deprecates using Kotlin language 1.7.
See the Kotlin release notes for other potential breaking changes.

==== The Kotlin DSL now uses the Kotlin language version 2.1

Gradle 8.x used the Kotlin language version 1.8.
Gradle 9.0 uses the Kotlin language version 2.1.
See the Kotlin release notes for a description of the language changes and available new language features.

The Kotlin 2 compiler makes change in the way scripts are compiled.
It is not possible to refer to the script object using `this@Build_gradle`, `this@Settings_gradle` or `this@Init_gradle` anymore.
You must prefer using the target object instead, respectively `project`, `settings` and `gradle`.

Kotlin 2.1 combined with link:#jspecify[JSpecify] brings some new strictness in how nullability is handled when consuming the Gradle API from the Kotlin DSL.
Here are some potential breaking changes.

Unbounded generics for types that have generic bounds will now fail to compile.
For example if you have a Kotlin extension function on `Provider<T>` whose signature is `fun <T> Provider<T>.some()` you must qualify `<T>` as either  `<T : Any>` because `T` isn't nullable on `Provider<T>`.
...
When building and publishing plugins written using the Kotlin DSL, they will be usable on Gradle >= 8.11 only.
This is because Gradle 8.11 is the oldest version that embeds Kotlin >= 2.0 which is capable of reading Kotlin metadata version 2.

If you want to target older versions of Gradle with a plugin written using the Kotlin DSL on Gradle 9.x you need to configure the target Kotlin language and api versions to 1.x.

For example, targeting Kotlin 1.7 which allow to apply the plugin on Gradle >= 6.8:
...
Please refer to Gradle's link:compatibility.html#kotlin[compatibility matrix] to learn which version of Kotlin is embedded in which Gradle version.

Plugins written with the Kotlin DSL and published with Gradle 7.x and 8.x were usable on Gradle >= 6.8 only.
...
==== Kotlin DSL gradle-enterprise plugin block extension has been removed ====

In `settings.gradle.kts` (Kotlin DSL), you were able to use `gradle-enterprise` in the plugins block to apply the Gradle Enterprise plugin with the same version as `gradle --scan`.
...
Therefore, the `gradle-enterprise` plugin block extension has been removed.
...
==== Referencing tasks and domain objects by `"name"()` is no longer supported in Kotlin DSL

Reference a task or other domain object by its name using the `"name"()` notation is no longer possible in Gradle 9.
...
==== `libraries` or `bundles` from dependency version catalogs in the `plugins {}` block no longer accessible in Kotlin scripts

Accessing `libraries` or `bundles` from dependency version catalogs in the `plugins {}` block of a Kotlin script has been deprecated in Gradle 8.1.
Gradle 9.0 completes their removal, attempting to use them will result in errors and the failure of the build.
...
The wrapper JAR has been changed to an executable JAR, which is a JAR with a `Main-Class` attribute.
The wrapper scripts now start the wrapper JAR using `-jar` instead of `-classpath` and a main class argument, so when updating the wrapper scripts the wrapper JAR must be updated as well.
This should happen automatically when running `gradle wrapper` or `gradlew wrapper`.
